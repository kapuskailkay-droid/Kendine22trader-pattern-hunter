...
# --- RENDER WEB SERVICE PORT DİNLEYİCİSİ (FAILED HATASINI ÇÖZEN KISIM) ---
class HealthCheckHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, format, *args):
        return  # Konsolu gereksiz loglarla doldurmaması için

def start_http_server():
    port = int(os.environ.get("PORT", 10000))
    server = HTTPServer(("0.0.0.0", port), HealthCheckHandler)
    print(f"🌐 HTTP Sunucu Port {port} uzerinde baslatildi (Render onayladi).")
...
def grafik_ciz(df_mum, sembol, tf_etiket, formasyon_adi, kirilan_seviye, tp1, tp2, sl):
    ...
    ax_main = axes[0]
    ax_main.set_title(f"KENDINE22TRADER | {sembol} ({tf_etiket}) - {formasyon_adi}", fontsize=12, fontweight='bold', color='#F4E07B', pad=12)
...
def retest_bul(df, hacim_orani):
    ...
    if len(df) >= 20:
        ...
        if direk >= 4.0 and any(closes[-4:-1] > flama_tavan) and any(lows[-2:] <= flama_tavan * 1.004) and (c_son > flama_tavan) and (c_son > o_son):
            return "🚩 BOGA BAYRAGI (RETEST ONAYLANDI 🚀)", "🟢 LONG", flama_tavan
    ...
    if len(df) >= 20:
        ...
        if direk <= -4.0 and any(closes[-4:-1] < flama_taban) and any(highs[-2:] >= flama_taban * 0.996) and (c_son < flama_taban) and (c_son < o_son):
            return "🚩 AYI BAYRAGI (RETEST ONAYLANDI 🩸)", "🔴 SHORT", flama_taban
    ...
    if len(tepe_idx) >= 2 and len(dip_idx) >= 2:
        ...
        if abs(t1-t2)/t1 <= 0.015 and d2 > d1:
            ...
            if any(closes[-4:-1] > direnc * 1.001) and any(lows[-2:] <= direnc * 1.004) and (c_son > direnc) and (c_son > o_son):
                return "📐 YUKSELEN UCGEN (RETEST ONAYLANDI 🚀)", "🟢 LONG", direnc
    ...
    if len(tepe_idx) >= 2 and len(dip_idx) >= 2:
        ...
        if abs(d1-d2)/d1 <= 0.015 and t2 < t1:
            ...
            if any(closes[-4:-1] < destek * 0.999) and any(highs[-2:] >= destek * 0.996) and (c_son < destek) and (c_son < o_son):
                return "📐 ALCALAN UCGEN (RETEST ONAYLANDI 🩸)", "🔴 SHORT", destek

    return None, None, None

def main():
    # Arka planda Render icin Port dinleyiciyi ayri thread'de baslat
    threading.Thread(target=start_http_server, daemon=True).start()
    print("🚀 KENDINE22TRADER 7/24 Kesintisiz Formasyon Motoru Baslatildi...")
                                ...
                                if len(closes) >= 30:
                                    ...
                                    if formasyon_adi:
                                        ...
                                        if sinyal_id not in hafiza:
                                            ...
                                            print(f"✅ Sinyal Gonderildi: {sinyal_id}")
                        except Exception:
                            pass
        except Exception as e:
