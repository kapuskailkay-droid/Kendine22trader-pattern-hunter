...
# Render Port Dinleyici (Ücretsiz Sunucu Onayı İçin)
class SimpleHandler(BaseHTTPRequestHandler):

  def do_GET(self):
    self.send_response(200)
    self.end_headers()
    self.wfile.write(b'KENDINE22TRADER Botu 7/24 Aktif!')
...
def retest_bul(df, hacim_orani):
  ...
  if len(dip_idx) >= 2:
    ...
    if (
      ...
      if (
          any(closes[-5:-1] > boyun * 1.002)
          and any(lows[-3:] <= boyun * 1.006)
          and (c_son > boyun)
          and (c_son > o_son)
          and hacim_orani >= 1.1
      ):
  ...
  if len(tepe_idx) >= 2:
    ...
    if (
      ...
      if (
          any(closes[-5:-1] < taban * 0.998)
          and any(highs[-3:] >= taban * 0.994)
          and (c_son < taban)
          and (c_son < o_son)
          and hacim_orani >= 1.1
      ):
  ...
  if len(df) >= 20:
    ...
    if (
        direk >= 4.0
        and any(closes[-4:-1] > flama_tavan)
        and any(lows[-2:] <= flama_tavan * 1.004)
  ...
  if len(df) >= 20:
    ...
    if (
        direk <= -4.0
        and any(closes[-4:-1] < flama_taban)
        and any(highs[-2:] >= flama_taban * 0.996)
  ...
  if len(tepe_idx) >= 2 and len(dip_idx) >= 2:
    ...
    if abs(t1 - t2) / t1 <= 0.015 and d2 > d1:
      ...
      if (
          any(closes[-4:-1] > direnc * 1.001)
          and any(lows[-2:] <= direnc * 1.004)
          and (c_son > direnc)
          and (c_son > o_son)
      ):
  ...
  if len(tepe_idx) >= 2 and len(dip_idx) >= 2:
    ...
    if abs(d1 - d2) / d1 <= 0.015 and t2 < t1:
      ...
      if (
          any(closes[-4:-1] < destek * 0.999)
          and any(highs[-2:] >= destek * 0.996)
          and (c_son < destek)
          and (c_son < o_son)
      ):
...
def main():
  ...
  print('🚀 KENDİNE22TRADER 7/24 Kesintisiz Motor Devrede...')
  mexc = ccxt.mexc(
  ...
  while True:
    try:
      ...
      for sembol in hedef_listesi:
        ...
        mexc_link = (
            'https://www.mexc.com/tr-TR/futures/'
            f"{temiz_parite.replace('/', '_')}"
        )
